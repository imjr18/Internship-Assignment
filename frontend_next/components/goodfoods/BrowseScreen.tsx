              ...
              <div style={{ flex: '0 0 60%' }}>
                ...
                <div style={{ marginTop: '20px' }}>
                  ...
                  <p style={{ fontSize: '14px', color: '#f7f3ee' }}>
                    {(() => {
                      const h = restaurant.operating_hours
                      // Simple format: {open, close}
                      if (typeof h.open === 'string' && typeof h.close === 'string') {
                        return `${h.open} → ${h.close}`
                      }
                      // Per-day format: {monday: {open, close}, ...}
                      const days = Object.entries(h)
                      if (days.length > 0) {
                        const first = days[0][1] as { open?: string; close?: string } | undefined
...
export function BrowseScreen({ onBookRestaurant, defaultOpenId = null }: BrowseScreenProps) {
  const { restaurants: LIVE_RESTAURANTS, loading } = useRestaurants()
  const ALL_CUISINES = useMemo(
    () => [...new Set(LIVE_RESTAURANTS.map((r) => r.cuisine_type))].sort(),
    [LIVE_RESTAURANTS]
  )
  const ALL_NEIGHBORHOODS = useMemo(
    () => [...new Set(LIVE_RESTAURANTS.map((r) => r.neighborhood))].sort(),
    [LIVE_RESTAURANTS]
  )
  ...
  const [open, setOpen] = useState<Restaurant | null>(
    defaultOpenId ? (LIVE_RESTAURANTS.find((r) => r.id === defaultOpenId) ?? null) : null
  )

  const filtered = useMemo(
    () =>
      LIVE_RESTAURANTS.filter((r) => {
    ...
    [cuisines, neighborhoods, prices, LIVE_RESTAURANTS]
  )
  ...
  return (
    <div style={{ paddingTop: '60px' }}>
      ...
      <FilterBar
        ...
        allNeighborhoods={ALL_NEIGHBORHOODS}
        cuisines={cuisines}
        neighborhoods={neighborhoods}
        prices={prices}
        count={filtered.length}
        total={LIVE_RESTAURANTS.length}
